...
def text_to_ids(source_text, target_text, source_vocab_to_int, target_vocab_to_int):
    ...
    source_id_text = [[source_vocab_to_int.get(word) for word in line.split()]  for line in source_text.lower().split('\n')]
    target_id_text = [[target_vocab_to_int.get(word) for word in line.split()]+[target_vocab_to_int['<EOS>']]  for line in target_text.lower().split('\n')]
...
def process_decoding_input(target_data, target_vocab_to_int, batch_size):
    ...
    """
    # TODO: Implement Function
    ending_part = tf.strided_slice(target_data, [0,0], [batch_size, -1], [1,1])
    decoding_input = tf.concat([tf.fill([batch_size,1], target_vocab_to_int['<GO>']), ending_part],1)
...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob):
    ...
    rnn_layer = tf.contrib.rnn.MultiRNNCell([drop_cell]*num_layers)
    _, enc_rnn_state = tf.nn.dynamic_rnn(rnn_layer, rnn_inputs, dtype=tf.float32)
...
def decoding_layer_train(encoder_state, dec_cell, dec_embed_input, sequence_length, decoding_scope,
    ...
    train_pred, _, _= tf.contrib.seq2seq.dynamic_rnn_decoder(drop_dec_cell, train_decode_fn, dec_embed_input, sequence_length, scope=decoding_scope)
...
def decoding_layer_infer(encoder_state, dec_cell, dec_embeddings, start_of_sequence_id, end_of_sequence_id,
    ...
    """
    # TODO: Implement Function
    drop_infer_cell = tf.contrib.rnn.DropoutWrapper(dec_cell, output_keep_prob=keep_prob)
    ...
    infer_logits, _, _ = tf.contrib.seq2seq.dynamic_rnn_decoder(drop_infer_cell, infer_decoder_fn, scope=decoding_scope)
...
def decoding_layer(dec_embed_input, dec_embeddings, encoder_state, vocab_size, sequence_length, rnn_size,
    ...
    dec_cell = tf.contrib.rnn.MultiRNNCell([dec_lstm_cell]*num_layers)
